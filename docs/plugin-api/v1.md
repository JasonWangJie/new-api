...
### Video capability profiles

`meta.videoProfiles` optionally declares capability-driven forms for models
bound to `openai_video`. Each model uses at most one profile. The host validates
that profile models belong to both `meta.models` and the plugin's video protocol
claim; plugins without profiles keep the generic video request form.

Mode names are `text_to_video`, `image_to_video`, `reference_to_video`,
`first_last_frame`, `edit_video`, and `extend_video`. Duration is either a
bounded `{min,max,step,default}` range or `{values,default}`; all values are
positive whole seconds and cannot exceed the host task-duration limit.
`duration`, `resolutions`/`defaultResolution`, and
`aspectRatios`/`defaultAspectRatio` are mode-level optional dimensions. A
present dimension is client-configurable. An absent dimension is inherited
from the source material or fixed by the provider, and the host rejects a
client value for it. Every declared default must belong to its list.

Inputs declare `{name,kind,sources,maxItems,required?}`. Kinds are `image`,
`video`, and `audio`; sources are `url`, `asset`, `data_uri`, `file_id`,
`voice_id`, and `upload`. The host checks positive input limits up to 64. Options advertise
whether a mode supports `generateAudio`, `watermark`, and `returnLastFrame`.
These profiles drive UI and host validation, but plugin `decodeRequest` remains
the final provider-specific validation boundary for conditional combinations.

`edit_video` and `extend_video` each declare exactly one required primary input
with `{name: "video", kind: "video", maxItems: 1, required: true}`. Extension
modes additionally declare `extensionDirections` containing `forward` and/or
`backward`, plus a member `defaultExtensionDirection`. The host exposes
`POST /v1/videos/edits` and `POST /v1/videos/extensions` only for models whose
profiles declare the corresponding mode; older plugins therefore keep only
the generation endpoint. Protocol decode receives operation `edit` or `extend`
and should return task actions `edit_video` or `extend_video`.

This is an additive API v1 extension. Older plugins remain valid; older strict
hosts reject the new `videoProfiles` field.
...
### Channel-managed asynchronous override

An administrator may configure `settings.upstream_async.profiles` on a channel. This is a host/channel contract, not plugin metadata, so it does not add fields to `meta`, the plugin JSON Schema, or the TypeScript declarations. Profiles select a media type, the model after channel mapping, and one or more normalized operations (`generate`, `edit`, `extend`).

When a profile matches, it atomically replaces the plugin driver's submit task-id extraction, polling request, status mapping, and primary result extraction. The plugin still owns request construction, request/model validation, billing hooks, completion-usage hooks, presenters, and any explicitly declared artifacts. An unmatched task follows the normal plugin or built-in adapter path without merging either parser. A matched synchronous image channel is treated as async-only for that selector, so a task receipt cannot be mistaken for an immediate image result.

The host freezes a secret-free copy of the selected profile with the persisted task. Later channel setting changes therefore do not alter an in-flight task. The upstream task id, raw submit/poll snapshot, and result URL stay in private task data; they are not exposed through `task.data` or public async queries. If a successful video response has no plugin artifact, the host synthesizes one video artifact from the configured primary URL and sends it through the existing download, validation, storage, and signed-result pipeline.

Configured status values retain their JSON scalar type; a string, number, and boolean with similar text are distinct. Missing or unknown status values produce `UNKNOWN` and use the same consecutive polling-failure budget described below. Result and usage parsing must succeed before a task completes. Image results may be a bounded string array; video has exactly one primary URL. Download headers containing channel credentials are used only for same-origin result URLs, while cross-origin results must be anonymously accessible HTTP(S) URLs.
...
`Task.Data` is normally the latest upstream response snapshot for presenters and artifacts. The host overwrites it on every successful plugin parse. Channel-managed asynchronous overrides are the exception: their raw response stays in private task data to prevent task ids and result URLs from leaking. Values that must survive across plugin poll rounds belong in `state`.
