...
export type VideoModeName = "text_to_video" | "image_to_video" | "reference_to_video" | "first_last_frame" | "edit_video" | "extend_video";
export type VideoExtensionDirection = "forward" | "backward";
export type VideoDuration = {min: number; max: number; step: number; default: number} | {values: readonly number[]; default: number};
export type VideoInput = {name: string; kind: "image" | "video" | "audio"; sources: readonly ("url" | "asset" | "data_uri" | "file_id" | "voice_id" | "upload")[]; maxItems: number; required?: boolean};
export type VideoModeProfile = {name: VideoModeName; duration?: VideoDuration; resolutions?: readonly string[]; defaultResolution?: string; aspectRatios?: readonly string[]; defaultAspectRatio?: string; extensionDirections?: readonly VideoExtensionDirection[]; defaultExtensionDirection?: VideoExtensionDirection; inputs?: readonly VideoInput[]; options?: {generateAudio?: boolean; watermark?: boolean; returnLastFrame?: boolean}};
export type VideoProfile = {models: readonly string[]; modes: readonly VideoModeProfile[]};
export interface Meta {requiredCapabilities?: readonly HostCapability[]; submitResponseTypes?: readonly ("json" | "sse")[]; sortPriority?: number; website?: string; apiVersion: 1; key: string; name: string; icon?: string; description?: LocalizedText; version: string; author: {name: string; url?: string}; baseUrl?: string; channelTypes?: readonly number[]; models: readonly string[]; fetchMode: "per_task" | "batch"; allowedHosts?: readonly string[]; routes?: readonly NativeRoute[]; protocols?: readonly ProtocolClaim[]; usageSchema?: Readonly<Record<string, UsageFieldSchema>>; usageExamples?: readonly UsageExample[]; usageProfiles?: readonly UsageProfile[]; videoProfiles?: readonly VideoProfile[]; auth?: "none" | "api_key" | "vertex_oauth" | {type: "none" | "api_key" | "oauth2_jwt"}}
