...
func (a *Adaptor) ImageCapability(model string) dto.ImageCapability {
	...
	return dto.ImageCapability{Models: models, Parameters: []string{"model", "prompt", "n", "size", "quality", "response_format", "provider_extensions"}, Provider: "gemini", Protocol: "gemini_native", Generate: model == "" || dto.ImageCapability{Models: models}.DeclaresModel(model), Edit: !strings.HasPrefix(strings.ToLower(model), "imagen")}
}
